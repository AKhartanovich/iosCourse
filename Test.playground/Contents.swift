...
/*
 Значимый тип данных означает, что выделяется новая ячейка памяти для переменной, значение копируется при присвоении его переменной или константе, ну или в функцию. Ну и выделяется при этом новая область памяти для значения.
 Например:
 */
struct PersonStruct {
    var Name: String
